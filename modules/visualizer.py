...
#         df = insights["top_products"].head(10)
#         fig, ax = plt.subplots(figsize=(10, 5))
#         bars = ax.barh(df['Description'][::-1], df['Revenue'][::-1],
#                        color=[ACCENT] + [ACCENT2] * (len(df) - 1))
#         _style_fig(fig, ax)
#         ax.xaxis.set_major_formatter(mticker.FuncFormatter(lambda x, _: f"£{x:,.0f}"))
#         ax.yaxis.set_major_formatter(mticker.FuncFormatter(lambda x, _: str(x)))
...
#         ax.plot(labels, revenues, color=ACCENT, linewidth=2.5, marker='o', markersize=5)
#         ax.fill_between(range(len(labels)), revenues, alpha=0.15, color=ACCENT)
...
#         df = insights["top_countries"].head(8).copy()

#         # ✅ Ensure correct datatype (prevents categorical warning)
#         df['Country'] = df['Country'].astype(str)

#         fig, ax = plt.subplots(figsize=(8, 4))

#         colors = [ACCENT, ACCENT2, "#f78166", "#ffa657", "#79c0ff", "#56d364", "#d2a8ff", "#ff7b72"]

#         # ✅ Use numeric positions instead of direct string categories
#         x = range(len(df))

#         ax.bar(x, df['Revenue'].values, color=colors[:len(df)])

#         # ✅ FIX: define ticks BEFORE labels
#         ax.set_xticks(x)
#         ax.set_xticklabels(df['Country'], rotation=30, ha='right', fontsize=8)
